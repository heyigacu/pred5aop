...
def plot_auc(labels, preds, savename="MLP",classnames=['a','b','c']):
    save_path = os.path.join(dir,'pretrained/{}_ROC_AUC.png'.format(savename))
    n_classes = labels.shape[1]
    ...
    plt.title("ROC curves of One-vs-Rest multi-classification", fontsize=13, fontweight='bold')
...
def multi_classify_metrics(labels, preds, average_method='weighted', plot_auc_curve=False, plot_confuse_matrix=False, savename="MLP",classnames=['','b','d','e','']):
    ...
    if plot_confuse_matrix:
        save_path = os.path.join(dir,'pretrained/{}_confusion_matrix.png'.format(savename))
        conf_matrix = pd.DataFrame(cm, index=classnames, columns=classnames)
        # plot size setting
        fig, ax = plt.subplots(figsize = (8,7))
        sns.heatmap(conf_matrix, annot=True, annot_kws={"size": 15}, cmap="Blues", fmt='.0f')
